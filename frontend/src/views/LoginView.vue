<template>
  <div class="min-h-screen flex items-center justify-center p-4 bg-slate-50">
    <div class="bg-white rounded-lg shadow-sm w-full max-w-md overflow-hidden border border-slate-200">
      <!-- Clean Solid Header (Green #4db03c) -->
      <div class="bg-[#4db03c] p-6 text-center text-white">
        <div class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded bg-white/15 text-[11px] font-medium text-emerald-50 mb-2.5 border border-white/20">
          <svg class="w-3.5 h-3.5 text-emerald-100 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
          <span>Industry–Academia Collaboration</span>
        </div>
        <h1 class="text-xl font-bold text-white tracking-tight">
          HITAM Live Project Tracker
        </h1>
        <p class="text-xs text-emerald-50 mt-1">Unified Multi-Tenant Workspace</p>
      </div>
      
      <div class="p-6">
        <div class="flex space-x-1 mb-6 bg-slate-100 p-1 rounded-md">
          <button @click="authMode = 'login'" :class="['flex-1 py-1.5 rounded text-xs font-semibold transition-all uppercase tracking-wide', authMode === 'login' ? 'bg-white text-[#4db03c] shadow-sm' : 'text-slate-600 hover:text-slate-900']">Student</button>
          <button @click="switchToRegister" :class="['flex-1 py-1.5 rounded text-xs font-semibold transition-all uppercase tracking-wide', authMode === 'register' ? 'bg-white text-[#4db03c] shadow-sm' : 'text-slate-600 hover:text-slate-900']">Register</button>
        ...
        <form v-if="authMode === 'login'" @submit.prevent="handleLogin" class="space-y-4">
          <div>
            ...
            <input type="text" required v-model="loginRoll" class="w-full rounded-md border border-slate-300 py-2 px-3 text-sm focus:ring-2 focus:ring-[#4db03c] focus:border-[#4db03c] outline-none" placeholder="e.g. 24E51A6634">
          </div>
          <div>
            <label class="block text-xs font-semibold text-slate-700 mb-1">Password</label>
            <div class="relative">
              <input :type="showLoginPass ? 'text' : 'password'" required v-model="loginPass" class="w-full rounded-md border border-slate-300 py-2 pl-3 pr-10 text-sm focus:ring-2 focus:ring-[#4db03c] focus:border-[#4db03c] outline-none" placeholder="••••••••">
          ...
          <button type="submit" :disabled="authStore.isLoading" class="w-full bg-[#4db03c] hover:bg-[#439c34] text-white font-semibold py-2.5 rounded-md transition-colors text-sm disabled:opacity-70">
            Login to Workspace
          </button>
        </form>
        ...
        <form v-if="authMode === 'register'" @submit.prevent="handleRegister" class="space-y-3.5">
          <div>
            ...
            <input type="text" required v-model="regName" class="w-full rounded-md border border-slate-300 py-2 px-3 text-sm focus:ring-2 focus:ring-[#4db03c] focus:border-[#4db03c] outline-none" placeholder="John Doe">
          </div>
          <div>
            <label class="block text-xs font-semibold text-slate-700 mb-1">Roll Number</label>
            <input type="text" required v-model="regRoll" class="w-full rounded-md border border-slate-300 py-2 px-3 text-sm focus:ring-2 focus:ring-[#4db03c] focus:border-[#4db03c] outline-none" placeholder="e.g. 24E51A6634">
          </div>
          ...
          <div>
            ...
            <select 
              ...
              required
              class="w-full rounded-md border border-slate-300 py-2 px-3 text-sm focus:ring-2 focus:ring-[#4db03c] focus:border-[#4db03c] outline-none bg-white text-slate-800 font-medium"
            >
          ...
          <div>
            <div class="flex items-center justify-between mb-1">
              ...
              <span v-if="isFetchingTeams" class="text-[10px] text-[#4db03c] font-medium">Loading teams...</span>
            </div>
            <select 
              v-model="regTeam" 
              :disabled="isFetchingTeams"
              class="w-full rounded-md border border-slate-300 py-2 px-3 text-sm focus:ring-2 focus:ring-[#4db03c] focus:border-[#4db03c] outline-none bg-white text-slate-800 disabled:bg-slate-50 disabled:text-slate-400"
            >
          ...
          <div>
            <label class="block text-xs font-semibold text-slate-700 mb-1">Password</label>
            <div class="relative">
              <input :type="showRegPass ? 'text' : 'password'" required v-model="regPass" class="w-full rounded-md border border-slate-300 py-2 pl-3 pr-10 text-sm focus:ring-2 focus:ring-[#4db03c] focus:border-[#4db03c] outline-none" placeholder="Create a password">
